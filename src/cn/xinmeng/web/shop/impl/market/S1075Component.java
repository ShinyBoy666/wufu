...
import cn.xinmeng.web.shop.entity.market.S1075ResponseEntity;
import cn.xinmeng.web.shop.entity.vo.BannerVo;

/**
 * 秒杀活动详商品列表（wap用）
 * @param 
 * @ClassName: S1075Component 
 * @version V1.0  
 ...
 *
 */
@Component ("cn.xinmeng.web.shop.impl.market.S1075Component")
public class S1075Component extends BaseService {
	...
	public AppResponseEntity callServer(AppRequestEntity request,String data){
		...
		S1075ResponseEntity result = new S1075ResponseEntity();
		...
		if(!Utils.isEmpty(list)){
			...
			for (ActivityProduct activityProduct : list) {
				...
				if(flag){
					ActvityProductInfoBiz biz = redisService.getActvityProductInfoBiz(activityProduct.getActivityId(), activityProduct.getActivityType(), activityProduct.getProductId(), activityProduct.getProductPriceId());
					result.setActivityState(biz.getActivityState());
					result.setActivityTime(biz.getActivityTime());
					result.setTitle(biz.getTitle());
					BannerVo vo = new BannerVo();
					vo.setImageUrl(biz.getActivityImage());
					bannerList.add(vo);
					flag = false;
				}
				resList.add(res);
